...
instance ToJSON MultiSigScript where
  toJSON (RequireSignature payKeyHash) = object [ "requiredSignature" .=  (String . Text.decodeUtf8 $ serialiseToRawBytesHex $ payKeyHash) ]
  toJSON (RequireAnyOf reqSigs) = object [ "requireAny" .= map toJSON reqSigs ]
  toJSON (RequireAllOf reqSigs) = object [ "requireAll" .= map toJSON reqSigs ]
...
parseMultiSigScript hms = case  all' hms <|> any' hms <|> atLeast hms of
 ...
 where
  ...
  convertValueToMultiSigScript (Object valueHm) =
    any' valueHm  <|> all' valueHm <|> atLeast valueHm <|> reqSig valueHm
  ...
  any' hm = case HMS.lookup "requireAny" hm of
             Just (Array anyValue) ->
               Just . RequireAnyOf $ gatherMultiSigScripts anyValue
             _ -> Nothing

  all' :: HMS.HashMap Text Value -> Maybe MultiSigScript
  all' hm = case HMS.lookup "requireAll" hm of
             Just (Array allVectorValue) ->
               Just . RequireAllOf $ gatherMultiSigScripts allVectorValue
             _ -> Nothing

  reqSig :: HMS.HashMap Text Value -> Maybe MultiSigScript
  reqSig hm = case HMS.lookup "requiredSignature" hm of
  ...
  atLeast hm = case HMS.lookup "atLeast" hm of
                 Just (Object atLeastHm) ->
                   case (HMS.lookup "required" atLeastHm, HMS.lookup "list" atLeastHm) of
                     (Nothing, Nothing) ->
